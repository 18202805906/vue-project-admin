<template>
  <div class="auth-content">
    <div class="height: 230px">
      <span>
        <span class="left-icon" @click="nextConfig" v-if="showLeftIcon">
          <left-outlined />
        </span>
        <a-list :loading="loading" :grid="{ gutter: 8, column: 3 }" :data-source="showData" class="list">
          <template #renderItem="{ item }">
            <a-list-item>
                <a-card :hoverable="true" :style="item.id == selectCardId ? { borderColor: '#1e93e7c2', borderWidth: '2px' } : { borderColor: '#eee', borderWidth: '1px' }" @click="clickCard(item)">
        ...
        <span class="right-icon" v-if="showRightIcon" @click="upConfig"><right-outlined /></span>
      </span>
    </div>
...
import srcPng from '@/assets/images/logo.png';
const rules = {
...
export default {
  ...
  setup(){
    ...
    const methods = {
      ...
      upConfig:()=> {
        //获取最后展示的一个card
        const lastCard = state.showData[state.showData.length - 1];
