      ...
      <a-form :form="form" @submit="handleSubmit">
        <a-form-item class="btn-box r-nw-fe-c">
          <div class="service-sign-in">
            ...
            <div>
              <a-button type="primary" :style="{marginRight: operation ==='add' ? '': '8px'}"  icon="down-circle" html-type="submit">保存</a-button>
              <a-popconfirm placement="topLeft" title="是否需要重置" okText="Yes" cancelText="No" @confirm="confirm">
                <a-button type="primary" ghost v-if="operation == 'add' ? true: false" class="btn-reset" icon="sync">重置</a-button>
              </a-popconfirm>
              <!-- <a style="cursor: pointer" @click="returnApiPage"><a-icon type="arrow-left" />返回</a> -->
              <a-button @click="returnApiPage" type="primary" icon="arrow-left" ghost>返回</a-button>
            </div>
          </div>
        </a-form-item>
        <a-form-item>
                  ...
                  <a-tabs defaultActiveKey="1" @change="callback">
                    <a-tab-pane tab="查询params" key="1">
                      <a-table class="tabel-box" :columns="paramsHeaderColumns" :data-source="paramsData" :pagination="false" rowKey="key">
                        <template slot="number" slot-scope="text, record">{{ record.key }}</template>
                        ...
                        <template slot="must" slot-scope="text, record">
...
const serverUrl = process.env.NODE_ENV === 'development' ? '/api' : process.env.VUE_APP_BASE_API;
const paramTyps = [
  { key: '0', value: '字节' },
  { key: '1', value: '字符' },
  { key: '2', value: '数字' },
...
export default {
  ...
  created() {
    ...
    this.form.cachedBind.paramsData = this.paramsData;
    this.form.cachedBind.headersData = this.headersData;
    this.form.cachedBind.formData = this.formData;
    this.form.cachedBind.formUrlencoded = this.formUrlencoded;
